...
export default function RegionalSupport({ gradeLabel, trackLabel, abilities, homeDistrict }: { gradeLabel: string; trackLabel: string; abilities: Ability[]; homeDistrict?: string }) {
  ...
  const featured = trpc.centres.featured.useQuery();
  const visibleDistricts = region === "全部地區" ? GROUPS.flatMap((group) => group.items) : GROUPS.find((group) => group.region === region)?.items ?? [];
  const activeCentres = useMemo(() => filterCentresByCoverage(featured.data ?? [], district, region), [featured.data, district, region]);
  const coveredDistricts = useMemo(() => new Set((featured.data ?? []).map((centre) => centre.district)), [featured.data]);
  const openWhatsApp = (topic: string, centreName?: string) => { if (!centralContact.data?.whatsapp) return; const area = district || region === "全部地區" ? district || "香港" : region; const text = centreName ? `你好，我想由學習航圖了解 ${centreName} 在${area}提供的 ${gradeLabel}${trackLabel}「${topic}」支援及轉介流程。` : `你好，我居住於${area}，想了解 ${gradeLabel}${trackLabel}「${topic}」的學習支援。若日後有合作中心，請通知我。`; window.open(`https://wa.me/${centralContact.data.whatsapp}?text=${encodeURIComponent(text)}`, "_blank", "noopener,noreferrer"); };
