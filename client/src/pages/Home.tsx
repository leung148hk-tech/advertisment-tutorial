...
import { PRIMARY_ENGLISH_FRAMEWORK, primaryEnglishCombinedDomains, type PrimaryEnglishGrade } from "@/data/primaryEnglishFramework";
...
export default function Home() {
  ...
  const primaryEnglishGrade: PrimaryEnglishGrade | null = gradeInfo?.stage === "小學" && trackId === "english" ? gradeInfo.id as PrimaryEnglishGrade : null;
  const primaryEnglishFramework = primaryEnglishGrade ? PRIMARY_ENGLISH_FRAMEWORK[primaryEnglishGrade] : null;
  const primaryEnglishDomains = primaryEnglishGrade ? primaryEnglishCombinedDomains(primaryEnglishGrade) : null;
  const primaryEnglishDescription = primaryEnglishFramework
    ? `${primaryEnglishFramework.progression} 本卷整合閱讀理解與寫作基礎；寫作字數教學目標約為 ${primaryEnglishFramework.writingOutputTarget}，題目評估寫作準備、語言選擇及修訂，不等同完整作文批改。`
    : "";
